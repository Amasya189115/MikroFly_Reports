...
namespace MikroFly_Reports.Views
{
    ...
    public partial class BloProdDetailsPage : ContentPage
    {
        int count;
        ...
        public BloProdDetailsPage(string machine)
        {
            InitializeComponent();
        ...
        private void GetInstantData()
        {
            ...
            try
            {
                ...
                SqlCommand sqlcomM1 = new SqlCommand("select TOP 1 OpT_IsEmriKodu 'JobOrder',sto_isim 'Code',per_adi+' '+per_soyadi 'Operator' from URETIM_OPERASYON_HAREKETLERI" +
                    ...
                    " WHERE OpT_bitis_tarihi<'19000101' and sto_model_kodu=3 and sto_uretici_kodu='" + module + "'", sqlcon);
        ...
        private void GetAchievements()
        {
            try
            {
                ...
                SqlCommand sqlcomM1 = new SqlCommand("select Isnull(sum(sth_miktar),0) from STOK_HAREKETLERI inner join STOKLAR on sto_kod=sth_stok_kod" +
                    " where YEAR(sth_create_date) =YEAR(GETDATE()) and sto_model_kodu=3 AND sto_uretici_kodu='"+ module + "'" +
                    " and sth_evraktip=7 and sth_tip=0 and sth_cins=7 and sto_anagrup_kod='PR2'", sqlcon);
                Achieved = float.Parse(sqlcomM1.ExecuteScalar().ToString(), CultureInfo.InvariantCulture.NumberFormat);
        ...
        private void GetObjectives()
        {
            ...
            try
            {
                ...
                while (sdr.Read())
                {
                    Objectives.Dialyzers = float.Parse(sdr[3].ToString(), CultureInfo.InvariantCulture.NumberFormat);
                    Objectives.Bloodllines = float.Parse(sdr[3].ToString(), CultureInfo.InvariantCulture.NumberFormat);
                    Objectives.Powders = float.Parse(sdr[3].ToString(), CultureInfo.InvariantCulture.NumberFormat);
                    Objectives.Needles = float.Parse(sdr[3].ToString(), CultureInfo.InvariantCulture.NumberFormat);
        ...
        private void CreateChart()
        {
            ...
            if(module== "NEPHROCAN-Ext")
            {
                objective = 0;
            }
            else
            {
                if(module== "NEPHROCAN-Assembly")
                {
                    objective = Objectives.Bloodllines * 2;
                }
                else
                {
                    objective = Objectives.Bloodllines;
                }
            }
            
            if(objective - Achieved<0)
            {
                obj = 0;
            }
            else
            {
                obj = objective - Achieved;
            }

            var entry = new ChartEntry(obj)
            {
                Label = "Remained",
                ValueLabel = (objective - Achieved).ToString(),
            ...
            if(objective==0)
            {
                LabelPieChartHeader.Text = DateTime.Now.Year.ToString() + " Plan: -";
            }
            else
            {
                LabelPieChartHeader.Text = DateTime.Now.Year.ToString() + " Plan: " + (objective).ToString();
            }
        ...
        private void GetMonthlyProduction()
        {
            ...
            try
            {
                ...
                SqlCommand sqlcom = new SqlCommand("select FORMAT(sth_create_date, 'MMMM'),MONTH(sth_create_date), Isnull(sum(sth_miktar),0) from STOK_HAREKETLERI inner join STOKLAR on sto_kod=sth_stok_kod" +
                    " where Year(sth_create_date) =Year(GETDATE()) and sto_model_kodu=3 and sto_uretici_kodu='"+module+"' and sth_evraktip=7 and sth_tip=0 and sth_cins=7 and sto_anagrup_kod='PR2'" +
                ...
                while (sdr.Read())
                {
                    MonthlyProductions.Add(new MonthlyProduction
                    {
                        ...
                        value = float.Parse(sdr[2].ToString(), CultureInfo.InvariantCulture.NumberFormat)
                    });
                }
                sdr.Close();
                sqlcon.Close();
            ...
            try
            {
                ...
                while (sdr.Read())
                {
                    MonthlyProductionWithProduct.Add(new MonthlyProductionWithProducts
                    {
                        ...
                        Desc = sdr[1].ToString(),
                        Lot = sdr[2].ToString(),
                        Value = float.Parse(sdr[3].ToString(), CultureInfo.InvariantCulture.NumberFormat)
                    });
                }
                sdr.Close();
                sqlcon.Close();
